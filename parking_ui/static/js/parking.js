...
function updateDefaultCamInfo(){
  fetch('/api/live/stats').then(r=>r.json()).then(s=>{
    ...
    let txt = '';
    if(defCam) txt = `Par défaut : ${defCam.name}`;
    else txt = 'Aucune caméra par défaut';
    if(s.detection_running && engCam){
...
async function liveStop(){
  ...
  img.src=''; img.style.display='none';
  const ov = document.getElementById('streamOverlay');
  ov.innerHTML = '<div class="so-title">FLUX VIDÉO ARRÊTÉ</div><div class="so-sub">Détection en cours en arrière-plan<br>Cliquez Démarrer pour voir le flux</div>';
  ov.classList.remove('hidden');
  document.getElementById('lStartBtn').disabled = false;
  document.getElementById('lStopBtn').disabled = true;
...
async function demoStop(){
  ...
  const ov = document.getElementById('demoOverlay');
  ov.innerHTML = '<div class="so-title">DÉMO ARRÊTÉE</div><div class="so-sub">Détection en direct relancée en arrière-plan</div>';
  ov.classList.remove('hidden');
  document.getElementById('dStartBtn').disabled = false;
