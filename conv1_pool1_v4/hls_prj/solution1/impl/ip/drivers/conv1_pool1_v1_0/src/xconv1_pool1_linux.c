...
typedef struct {
    ...
    XConv1_pool1_uio_map maps[ MAX_UIO_MAPS ];
} XConv1_pool1_uio_info;

/***************** Variable Definitions **************************************/
static XConv1_pool1_uio_info uio_info;
...
static int uio_info_read_name(XConv1_pool1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConv1_pool1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConv1_pool1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConv1_pool1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConv1_pool1_Initialize(XConv1_pool1 *InstancePtr, const char* InstanceName) {
	XConv1_pool1_uio_info *InfoPtr = &uio_info;
